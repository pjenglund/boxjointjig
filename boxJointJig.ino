...
MyClass StepperMotor(pulPin, dirPin, maxSpeed, mmPerRev, stepsPerRev);
...
void loop() {
  ...
  if (digitalRead(homeButton) == LOW && homeButtonPressed == true) homeButtonPressed = false;
    ...
    switch (cutNumber) {
      case 1:
        ...
        if (slotFirstPosition != 0) {
          ...
          if (DO_LOG) Serial.println(StepperMotor.stepsToMm(StepperMotor.getPosition()));
        }
      ...
      case 2:
        ...
        if (slotFirstPosition > (StepperMotor.getPosition() - StepperMotor.mmToSteps(bladeKerf * 0.9))) cutNumber = 0;
        break;
      default:
  ...
  if (digitalRead(moveButton) == LOW && moveButtonPressed == true) moveButtonPressed = false;
}
...
void testSteps(int steps) {
  StepperMotor.stepSteps(steps);
}

void testMm(float mm) {
  StepperMotor.stepMm(mm);
}
